"""
core/daily_report.py — Daily summary report generator and scheduler.
...
PostgreSQL: uses execute_read() instead of the old get_connection() pattern.
Humidity references removed (temperature-only system).
"""
...
async def generate_daily_report() -> dict:
    ...
    Returns a dict with:
      ...
      avg_temperature, peak_temperature, peak_temperature_time,
    ...
    try:
        ...
        readings = execute_read(
            ...
            "WHERE timestamp >= %s AND is_valid = 1",
            (today,),
        )
        ...
        if readings:
            ...
            peak_temp_row = max(readings, key=lambda r: r["temperature"])

        # ── Alert stats ───────────────────────────────────────────────────────
        alerts = execute_read(
            "SELECT parameter, direction FROM alerts WHERE timestamp >= %s",
            (today,),
        )
        ...
        # ── Escalation stats ──────────────────────────────────────────────────
        esc_rows = execute_read(
            "SELECT COUNT(*) AS cnt FROM escalation_log WHERE sent_at >= %s",
            (today,),
        )
        total_escalations = esc_rows[0]["cnt"] if esc_rows else 0
